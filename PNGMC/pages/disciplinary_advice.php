...
$disciplinary_advices = [];
$students = [];

// Handle form submissions
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $conn) {
    $tables_exist = $conn->query("SHOW TABLES LIKE 'disciplinary_advice'")->num_rows > 0;
    ...
    if ($tables_exist) {
        if (isset($_POST['create_advice'])) {
            ...
            if ($student) {
                ...
                $year = date('Y');
                $prefix = "DA-{$year}-";
                $result = $conn->query("SELECT advice_number FROM disciplinary_advice WHERE advice_number LIKE '{$prefix}%' ORDER BY disciplinary_id DESC LIMIT 1");
                ...
                $stmt = $conn->prepare("INSERT INTO disciplinary_advice (
                    advice_number, student_id, student_name, student_address, program_course_name, program_course_fee,
                ...
                $stmt->bind_param("sissdssssdsssddssi",
                    $advice_number, $student_id, $student_name, $student_address, $program_course_name, $program_course_fee,
                    $reason, $reason_details, $action_taken, $fee_amendment, $amendment_type,
                    $start_date, $ending_date, $paid_fees, $balance, $remarks, $created_by
                );
                ...
                if ($stmt->execute()) {
                    ...
                    if ($_SESSION['role'] === 'studentservices' || $_SESSION['role'] === 'admin') {
                        ...
                        createWorkflowNotification(
                            ...
                            'action_required',
                            'disciplinary_advice.php',
                            $_SESSION['user_id']
                        );
                    }
                    
                    $message = "Disciplinary Advice created successfully! Advice Number: $advice_number" . 
                        ($_SESSION['role'] === 'studentservices' || $_SESSION['role'] === 'admin' ? "<br><br>✅ Finance has been notified." : "");
                    $message_type = "success";
                } else {
                    $message = "Error creating Disciplinary Advice: " . $stmt->error;
                    $message_type = "error";
                }
                $stmt->close();
            }
...
if ($conn) {
    ...
    if ($tables_exist) {
        $result = $conn->query("SELECT * FROM disciplinary_advice ORDER BY created_at DESC LIMIT 100");
        if ($result) {
            $disciplinary_advices = $result->fetch_all(MYSQLI_ASSOC);
        }
    }
...
<head>
  ...
  <title>Disciplinary Advice - Student Admin Services</title>
...
<body>
    ...
    <nav class="sidebar" aria-label="Main navigation">
      ...
      <div class="menu">
        ...
        <a class="menu-item active" href="disciplinary_advice.php">Disciplinary Advice</a>
    ...
    <div class="content">
      <header style="margin-bottom: 30px;">
        <h1>Disciplinary Advice</h1>
        <p class="small">
          <?php if ($_SESSION['role'] === 'studentservices' || $_SESSION['role'] === 'admin'): ?>
            Issue disciplinary advice for students and track fee amendments. Finance will be notified when advice is created.
          <?php else: ?>
            View disciplinary advice issued by Student Admin Service. These are sent to Finance for review and financial record updates.
          <?php endif; ?>
        </p>
      </header>
      ...
      <div class="main-card" style="margin-bottom: 30px;">
        <h2>Create Disciplinary Advice</h2>
        <form method="POST">
          <div class="form-grid">
            ...
            <div class="form-group">
              <label>Reason for Disciplinary *</label>
            ...
            <div class="form-group full-width">
              ...
              <textarea name="reason_details" rows="3" placeholder="Provide details about the disciplinary issue..."></textarea>
            </div>
            <div class="form-group">
          ...
          <button type="submit" name="create_advice" class="btn btn-primary">Create Disciplinary Advice</button>
        </form>
      </div>

      <div class="main-card">
        <h2>Disciplinary Advices</h2>
        <?php if (!empty($disciplinary_advices)): ?>
          <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
            ...
            <tbody>
              <?php foreach ($disciplinary_advices as $advice): ?>
        ...
        <?php else: ?>
          <p style="color: #666; margin-top: 20px;">No disciplinary advices found.</p>
        <?php endif; ?>
      </div>
    </div>
  </div>
